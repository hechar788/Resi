...
export default function BottomMenu() {
  ...
  return (
    <NavigationMenu
      viewport={false}
      className="fixed inset-x-0 bottom-0 z-40 w-full max-w-full border-t bg-white/95 px-2 py-2 backdrop-blur md:py-2"
    >
      <NavigationMenuList className="grid w-full grid-cols-4 gap-1 [@media(min-width:400px)]:gap-1.5 md:gap-4 lg:gap-5">
        {navItems.map((item) => {
          const isActive =
            pathname === item.to ||
            ...
            <NavigationMenuItem key={item.to} className="flex-1 min-w-0">
              <NavigationMenuLink asChild active={isActive}>
                <Link
                  to={item.to}
                  className={cn(
                    'flex h-full w-full flex-col items-center gap-1 rounded-xl px-5 py-2 text-xs font-medium transition-all md:px-8 md:py-2 md:text-sm',
                    isActive
